...
**Windows 桌面 Codex 灵动岛 · 红绿灯状态 + 液态玻璃额度球**
...
**Codex Open Island** 是一个常驻 Windows 系统托盘的桌面悬浮小组件，专门为 **Codex** 用户设计。它把 Codex 项目的运行状态、剩余额度、会话列表都汇聚到屏幕顶部一个精致的"灵动岛"上。
...
| 信号 | 灯色 | 闪烁 | Codex 事件 |
|------|:--:|------|-----------|
| **Ready** 就绪 | 🟢 | 常亮 | `sessionstart` `idle` |
| **Thinking** 思考中 | 🟢 | ⚡ 快闪 | `reasoning` `task_started` |
| **Working** 工作中 | 🟢 | 🐢 慢闪 | `function_call` `pre_tool_use` |
| **ToolDone** 步骤完成 | 🟢 | 🐢 慢闪 | `function_call_output` `post_tool_use` |
| **Permission** 需审批 | 🔴 | ⚡ 快闪 + 🏝弹跳 | `approval_requested` `permission_request` |
| **Attention** 需关注 | 🟡 | 🐢 慢闪 | `notification` `needs_review` |
| **Blocked** 阻塞/错误 | 🔴 | ⚡ 快闪 | `failure` `error` `turn_aborted` |
| **Completed** 已完成 | 🟢 | 常亮 + 🏝弹跳 | `task_complete` `final_answer` |
| **Stale** 状态过期 | 🟡 | 🐢 慢闪 | 超过 10 分钟未更新 |
| **Paused** 已暂停 | ⚫ | 熄灭 | `pause` `paused` |

> 💡 **完成 / 审批时**：灵动岛弹出到前台并持续跳动。点击岛体任意位置即停止跳动并确认状态。

## 🌐 额度球颜色规则

| 状态 | 颜色 | 条件 |
|------|:--:|------|
| **Healthy** 健康 | 🟢 `#32D74B` | 剩余 ≥ 10% |
| **Low** 偏低 | 🟡 `#FFD60A` | 0 < 剩余 < 10% |
| **Empty** 耗尽 | 🔴 `#FF453A` | 剩余 = 0% |
| **Loading** 加载中 | ⚫ 灰 | 初始读取 |
| **Error** 错误 | 🔴 红 | 读取失败（超时/未登录） |
| **Stale** 过期 | ⚫ 灰 | 数据超出新鲜窗口 |
| **Unknown** 未知 | ⚫ 灰 | 第三方模型无额度数据 |
...
本项目从以下三个优秀的开源项目中汲取了大量灵感和设计：

- **[codex-led-widget](https://github.com/xicunwus2025-sys/codex-led-widget/)** —— Codex 额度读取的 JSON-RPC stdio 协议、液态玻璃额度球视觉、红/黄/绿额度颜色映射。感谢作者提供了完整的本地 Codex 额度读取方案
- **[Agent-Signal-Bar-Windows](https://github.com/ridyang/Agent-Signal-Bar-Windows/)** —— AI Agent 红绿灯信号分类体系、信号聚合优先级、Windows 托盘集成思路。感谢作者设计了优雅的 Agent 状态语言
- **[open-island-windows](https://github.com/ludiwangfpga/open-island-windows/)** —— WPF 透明悬浮窗动态灵动岛框架、展开/折叠交互模式、系统状态监控、Windows 打包实践。感谢作者提供了完整的 Windows 灵动岛参考实现
